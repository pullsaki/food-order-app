...
const isNotSixChars = (value) => value.trim().length !== 5;
...
const Checkout = (props) => {
  ...
  const confirmHandler = (event) => {
    ...
    const enteredPostalIsValid =
      isNotSixChars(enteredPostal) && !isEmpty(enteredPostal);
  ...
  const streetControlClasses =
    classes.control + " " + (formInputsValidity.street ? "" : classes.invalid);

  const cityControlClasses =
    classes.control + " " + (formInputsValidity.city ? "" : classes.invalid);

  const postalControlClasses =
    classes.control + " " + (formInputsValidity.postal ? "" : classes.invalid);
